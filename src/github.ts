...
export default class GitHubClient {
  ...
  async countOfPublicGistsOfUser(user: string) {
  ...
  async howLongGithubUserCreatedInYears(user: string): Promise<number> {
    const userGetResponse = await this.octokit.rest.users.getByUsername({
      username: user,
    });
  ...
  async timeOfRepositoryLastUpdated(owner: string, repository: string) {
  ...
  async countOfCommitsAfterDate(user: string, start: Date) {
    const q = `author:${user}+author-date:>=${
      start.toISOString().split("T")[0]
    }`;
    const searchCommitsResponse = await this.octokit.rest.search.commits({
      q: q,
    });
  ...
  async countOfIssueOrPRsAfterDate(
    ...
    const searchIssuesAndPRsResponse =
      await this.octokit.rest.search.issuesAndPullRequests({ q: q });
  ...
  query { 
    user(login: "${user}") {
      ...
      const years = response.user.contributionsCollection
        .contributionYears as Array<number>;
      const results = await Promise.all(
        years.map(async (year) => {
          const yearStart = new Date(`${year}-01-01`);
          const yearEnd = new Date(`${year}-12-31`);
          return await this.countOfUserContributionsAfterDate(
            user,
            yearStart,
            yearEnd,
            false
  ...
  query { 
    ...
    const result = (response as any).user.contributionsCollection
      .contributionCalendar.totalContributions as number;
    return result;
  }
}
